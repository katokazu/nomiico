...
// MVP: gacha/swipe/roulette + 単一端末回し決め vote/ranking。draft/tournament は将来
type DecisionMode =
  | 'gacha' | 'swipe' | 'roulette' | 'vote' | 'ranking' | 'draft' | 'tournament';
type SessionStatus = 'active' | 'completed' | 'cancelled';
type SwipeResult = 'kept' | 'rejected' | 'pending';
type Revisit = 'yes' | 'meh' | 'no';            // また行きたい / うーん / もういい
type NotifPermission = 'unasked' | 'granted' | 'denied' | 'deferred';
...
interface Visit {
  ...
  companion?: string;
  revisit?: Revisit;
  photoUri?: string;           // ローカル写真 URI（MVP 1 枚）
  createdAt: string;
}
...
interface UserSettings {
  ...
  notifyMaxPerDay: number;     // 既定 1
  resurfaceAfterDays: number;  // 既定 90
...
`AddVisitInput` は `restaurantId` 必須、`rating`/`memo`/`companion`/`revisit`/`photoUri`/`decisionSessionId` は任意（星のみで成立）。
...
| `updateVisit(id, patch): Promise<Visit>` | 後から評価・メモ・写真・同行者を追記 |
| `findPendingRecordPrompt(): Promise<DecisionSession \| null>` | 決定済みで未記録のセッション（記録うながしバナー用）。`visits.decision_session_id` 未参照 & `record_prompt_dismissed_at` NULL |

## SettingsRepository

設定画面・通知（[home-and-decision-ux](../specs/home-and-decision-ux.md) §設定、[resurfacing](../specs/resurfacing.md)）。

| メソッド | 用途 |
|---|---|
| `getSettings(): Promise<UserSettings>` | 取得（初回は既定値をシード） |
| `updateSettings(patch): Promise<UserSettings>` | 通知 ON/OFF・時間帯・頻度・寝かせ日数の更新 |
| `setNotifPermission(status: NotifPermission): Promise<void>` | 許可シートの結果反映（granted/denied/deferred） |
...
| メソッド | 用途 |
|---|---|
| `startSession(mode, filter, opts?): Promise<DecisionSession>` | セッション開始（候補をスナップショット保存）。vote/ranking は `opts.participantCount` を渡す |
...
| `castVote(sessionId, restaurantId): Promise<void>` | みんなで・投票制: 1 票を加算（候補の `tally`++） |
| `castRanking(sessionId, ranked: string[]): Promise<void>` | みんなで・順位制: 並べた順に加点（上位 3/2/1 等）を `tally` に加算 |
| `dismissRecordPrompt(sessionId): Promise<void>` | 記録うながしを閉じる（`record_prompt_dismissed_at` 設定） |
