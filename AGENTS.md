...
## 守るべき原則
- 基本的に日本語でドキュメントを記述すること
- ファイル名やソースコード、プログラム的な値や固有名詞、変数名などは英語で構わない。
- 文字エンコーディングはUTF-8とすること。
- 改行コードはLFとすること。
...
実装済み（`imports`＝CSV 取り込みのみ後続フェーズとして未実装）。保存/手動登録（共有シート受信・
テキスト内 URL 抽出・手動登録編集フォーム・保存タブ一覧）は実装済み。ホーム今日の一店・ガチャ・
